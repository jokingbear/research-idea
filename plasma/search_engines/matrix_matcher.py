...
class GraphMatcher(AutoPipe):
    ...
    def __init__(self, texts, group_splitter=r'([^:\n,;?!.]+)', tokenizer=r'(\w+)',
        ...
        self.top_k = top_k
        self._data = self.build_columns(texts)
        self._db_tokens = self._data['tokens'].map(sum, start=[])
        self.path_walker = PathWalker(path_threshold, self._graph, self._data)

    def build_columns(self, texts:pd.Series):
        standardized_texts = texts.str.lower()
        data = []
        for txt in standardized_texts:
            path = self.tokenizer.run(txt)['token'].tolist()
            data.append([txt, path])

        data = pd.DataFrame(data, columns=['text', 'tokens'])
        return data
    ...
    def _analyze_group(self, group: str):
        ...
        token_matching_matrix = self._compare_tokens(group_tokens)


    def _compare_tokens(self, tokens):
        matrix = []
        for t in tokens:
            scores = [difflib.SequenceMatcher(None, a=t, b=db_token).ratio() for db_token in self._db_tokens]
            matrix.append(scores)

        matrix = pd.DataFrame(matrix, index=tokens, columns=self._db_tokens)
